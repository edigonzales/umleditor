 ...
 * like: - NavigationTree - DescriptionPanel - LogView - DrawingArea
 *
...
public class LauncherView extends BaseFrame implements MetaModelListener, DrawingEditor, PaletteListener,
	...
	public static final NlsString TAGGEDVALUE_CONFIG_DEFAULTMODELLINGLANGUAGE = new NlsString("en",
			"org.umleditor.config.defaultModellingLanguage");
	...
	private void adaptTools(DrawingFrame frame) {
		...
		if (frame == null) {
			...
			if (frame.getDrawingView() instanceof ClassDiagramView) {
				...
				tool.setUsable(true);
				defaultToolButton = createToolButton(UML_IMAGES + "SEL", getResourceString("CISelectionTool_text"), //$NON-NLS-1$ //$NON-NLS-2$
						tool);
				...
				tool = new CreationTool(this, new NoteFigure()) {
					public void mouseDrag(java.awt.event.MouseEvent e, int x, int y) {
						// don't track drag events during creation: figure size
						// is
				...
				if (classDiagram.allowsPackages()) {
					...
					tool = new CreationTool(this, new PackageFigure()) {
						public void mouseDrag(java.awt.event.MouseEvent e, int x, int y) {
							// don't track drag events during creation: figure
							// size is
							// calculated and independent of current size
						}
					};
				...
				if (classDiagram.allowsClasses()) {
					...
					tool.setUsable(true);
					palette.add(createToolButton(UML_IMAGES + "ASSOC_SELF", getResourceString("CIAssociationSelf_text"), //$NON-NLS-1$ //$NON-NLS-2$
							tool));

					// Generalization (acc. to Rose)
					tool = new ConnectionTool(this, new GeneralizationLineConnection());
					tool.setUsable(true);
					palette.add(createToolButton(UML_IMAGES + "INHERITANCE", getResourceString("CIInheritance_text"), //$NON-NLS-1$ //$NON-NLS-2$
							tool));
				}
	...
	private void addInternalFrame(ClassDiagramView drawingView) {
		...
		// TODO Patch: call getMinumSize() in ClassDiagramView-Constructor ->
		// Resize necessary because of JHotDraw (Bug?) here
	...
	private javax.swing.JMenuItem getMniExportXmi() {
		if (ivjMniExportXmi == null) {
			try {
				...
				ivjMniExportXmi.setText("Exportieren...");
				// user code begin {1}
	...
	private JPanel getTreProjects() {
		// There must be a hideen connection "connEtoC1" which cannot be removed
		// properly
	...
	private void initialize() {
		...
		setSize(getSettings().getWindowWidth().intValue() + insets.left + insets.right,
				getSettings().getWindowHeight().intValue() + insets.top + insets.bottom);
		initPlugins();
		// user code end
	}
	...
	public static void main(java.lang.String[] args) {
		...
		try {
			...
			// ce2004-06-23 don't change default Locale!!! use (existing) system
			// administration tool!
			// Locale.setDefault(new Locale(settings.getLanguage(),
			// settings.getCountry()));
			// Tracer.getInstance().runtimeInfo("Locale might have changed to: "
			// + java.util.Locale.getDefault().toString());

			// ce2010-06-04 distinguish use interface langauge and langugage of
			// model-element-names
			// use getSettings().getLanguage() for the user interface language
			// use ch.ehi.basics.types.NlsString.getDefaultLanguage() for the
			// language of model-element-names
			// (use of NlsString.getDefaultLanguage() is not fully correct,
			// because the default language of a
			// model-element depends on the containing ModelDef-Element and not
			// an application global setting!)
			...
			for (int i = 0; i < args.length; i++) {
				if (args[i].equals("-trace") || args[i].equals("-debug")) {
					EhiLogger.getInstance().setTraceFilter(false); // default
																	// config
																	// is:
																	// filter
																	// trace
			...
			instance.getSppMain().setDividerLocation(
					getSettings().getWindowHeight().intValue() - getSettings().getLogHeight().intValue());
			instance.getSppControl().setDividerLocation(
					getSettings().getWindowHeight().intValue() - getSettings().getDescriptionHeight().intValue());
			...
			BaseDialog.showError(instance, ResourceManager.getResource(LauncherView.class, "CTStartupError"), //$NON-NLS-1$
					"in main()", exception);//$NON-NLS-1$
			System.exit(-1);
		}
	}

	private static void logModellingLanguage() {
		EhiLogger.logState("default language of model-elementnames set to '"
	...
	public void metaModelChanged(MetaModelChange event) {
		...
		/*
		 * // adapt Window title if (event.getSource() instanceof
		 * ch.ehi.uml1_4.implementation.UmlModel) { setTitle(); }
		 */
	...
	/**
	 * The user selected a palette entry. The selected button is passed as an
	 * argument.
	 * 
	 * @see PaletteListener
	 */
	...
	/**
	 * Show refreshed documentation of the currently selected Element in
	 * Navigationtree.
	 *
	 */
	private void refreshDocumentation() {
		if (currentElement == null) {
			...
			if ((getPnlDocumentation().getText() != null) && (!getPnlDocumentation().getText()
					.equals(ElementUtils.mapNlsString(currentElement.getDocumentation())))) {
	...
	private void saveDocumentation() {
		...
		if ((currentElement != null) && getPnlDocumentation().hasContentsChanged()) {
			currentElement.setDocumentation(new ch.ehi.basics.types.NlsString(currentElement.getDocumentation(),
					getPnlDocumentation().getText()));
		}
	}
	...
	private boolean saveOnClosing() {
		if (saveCurrentChanges()) {
			...
			// getSettings().setLogHeight(new
			// Integer((int)dimension.getHeight()));
	...
	*/
	...
	private void setModel(Model model /* , java.io.File file */) throws Throwable {
		...
		if (model == null) {
			...
			TaggedValue defaultModellingLanguage = (TaggedValue) ElementFactory
			...
			try {
				new ch.ehi.umleditor.interlis.iliimport.TransferFromIli2cMetamodel()
						.loadPredefinedIli2cModel(this.model);
			} catch (Exception ex) {
				handleException(ex);
			}
		} else {
	...
	/**
	 * Show the Specification Dialog of the ModelElement. The Dialog will be
	...
	private void toggleStandardToolbar() {
		if (getMncToolbar().isSelected()) {
			...
			getSppMain().getTopComponent().setBounds(0, 0, getSppMain().getWidth(),
					getSppMain().getTopComponent().getHeight() - deltaHeight);
		} else {
			int deltaHeight = getTlbStandard().getHeight();
	...
	public void openFile(String filename) {
		try {
			...
			BaseDialog.showWarning((java.awt.Component) this,
					ResourceManager.getResource(FileHistoryMenu.class, "CTFileMissing"),
