...
public class AttributeDefDialog extends BaseDialog {
	...
	public AttributeDefDialog(java.awt.Frame owner, ch.ehi.uml1_4.foundation.core.Element element) {
		...
		initialize();
		addEscapeKey();
		addUndoRedo(getTxtName(), getTxtMetaAttrbAt());
	...
	private void adaptType() {
		...
		if (currentDataPanel != newPanel) {
			...
			if (currentDataPanel != null) {
				...
				getTbpGeneral().setTitleAt(getTbpGeneral().getTabCount() - 1,
						resAttributeDefDialog.getString("CTTypeSpecific")); //$NON-NLS-1$
	...
	private javax.swing.JPanel getBaseDialogContentPane() {
		if (ivjBaseDialogContentPane == null) {
			try {
				...
				constraintsTxtMetaAttrbAt.gridx = 1;
				constraintsTxtMetaAttrbAt.gridy = 0;
	...
	private javax.swing.JPanel getPnlConstraints() {
		if (ivjPnlConstraints == null) {
			try {
				...
				constraintsPnlSyntax.gridx = 0;
				constraintsPnlSyntax.gridy = 0;
	...
	private javax.swing.JTabbedPane getTbpGeneral() {
		if (ivjTbpGeneral == null) {
			try {
				...
				ivjTbpGeneral.insertTab(resAttributeDefDialog.getString("TbpRestriction_text"), null,
						getPnlConstraints(), null, 2);
		...
		if (getCbxType().getSelectedItem() != IliBaseTypeKind.getNullType()) {
			...
			if (getCbxType().getSelectedItem() == IliBaseTypeKind.DOMAINDEF) {
				...
				if (((DataPanel) currentDataPanel).getObject() != null) {
					domainAttribute.attachDomainDef(
							(ch.ehi.interlis.domainsandconstants.DomainDef) ((DataPanel) currentDataPanel).getObject());
				}
			} else {
				// or direct-type
				if (getCbxType().getSelectedItem() == IliBaseTypeKind.BOOLEAN) {
					...
					domainAttribute.attachDirect(
							(ch.ehi.interlis.domainsandconstants.Type) ((DataPanel) currentDataPanel).getObject());
				}
			}
			...
			if (attrType instanceof DomainAttribute) {
				...
				if (((DomainAttribute) attrType).containsDirect()) {
					...
					if (((DomainAttribute) attrType).containsDomainDef()) {
						...
						Tracer.getInstance()
								.runtimeWarning("DomainAttribute contains neither DirectType nor DomainDef");
					}
